...
- The submitted `grandpa.noteStalled` call used
  `best_finalized_block_number = 2_119_795`, which is the finalized height
  where finality had stopped.
...
For `last_finalized`, use the finalized reference from the forced recovery
itself, and verify it against the submitted `noteStalled` parameters. On
`gtest`, this means the value should be checked against
`best_finalized_block_number = 2_119_795` instead of relying on a later derived
height.

Using `#2188861` with `last_finalized: Some(...)` causes sync failures
