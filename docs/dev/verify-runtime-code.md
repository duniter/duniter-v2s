...
```bash
cat release/srtool_output_<network>.json | grep blake2_256 | jq -r '.runtimes.compressed.blake2_256'
```

Le `grep` est nécessaire car le fichier srtool contient des lignes non-JSON.
...
3. Vérifiez que la proposal référence bien une preimage contenant
   `upgradeOrigin.dispatchAsRoot(system.setCode(...))`
